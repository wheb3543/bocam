...
describe('TeamsPage - مساحة عمل التسويق الرقمي المدمجة', () => {
  const digitalMarketingSource = readSourceFile(
    resolve(process.cwd(), 'client/src/apps/admin/modules/06-tasks-projects/teams/DigitalMarketingTeamPage.tsx')
  );
  const underDevelopmentSource = readSourceFile(
    resolve(process.cwd(), 'client/src/core/pages/UnderDevelopmentPage.tsx')
  );
...
describe('TeamsPage - مساحة عمل فريق الوسائط المدمجة', () => {
  const mediaTeamSource = readSourceFile(
    resolve(process.cwd(), 'client/src/apps/admin/modules/06-tasks-projects/teams/MediaTeamPage.tsx')
  );
  const mediaStatsSource = readSourceFile(
    resolve(process.cwd(), 'client/src/apps/admin/modules/06-tasks-projects/teams/media/MediaStats.tsx')
  );
  const kanbanColumnSource = readSourceFile(
    resolve(process.cwd(), 'client/src/apps/admin/modules/06-tasks-projects/teams/media/KanbanColumn.tsx')
  );
