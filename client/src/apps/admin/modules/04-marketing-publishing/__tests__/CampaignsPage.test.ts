...
describe('CampaignsPage - مساحة العمل الداخلية', () => {
  const pageSource = readSourceFile(
    resolve(process.cwd(), 'client/src/apps/admin/modules/04-marketing-publishing/pages/CampaignsPage.tsx')
  );
  const overviewSource = readSourceFile(
    resolve(process.cwd(), 'client/src/apps/admin/modules/04-marketing-publishing/components/CampaignOverviewCards.tsx')
  );
