...
import { Search, TrendingUp, Download, X, Filter, Printer } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <Button
          variant={
            statusFilter.includes('new') && statusFilter.length === 1 ? 'default' : 'outline'
          }
          size="sm"
          onClick={() =>
            onStatusFilterChange(
              statusFilter.includes('new') && statusFilter.length === 1 ? [] : ['new']
            )
          }
          className={`gap-2 h-9 ${
