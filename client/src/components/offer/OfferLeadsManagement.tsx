...
export default function OfferLeadsManagement({
  ...
  const getOfferExportOptions = useCallback(() => {
    ...
    selectedOffer,
    dateRange,
    offerTable.visibleColumns,
    offerExport,
  ]);

  const handleExportOfferLeads = useCallback(
    ...
    <div className="space-y-6">
      ...
      <div className="hidden md:block rounded-lg border bg-card">
        <ResizableTable
          frozenColumns={offerTable.frozenColumns.frozenColumns}
          columnWidths={offerTable.columnWidths.columnWidths}
          visibleColumnOrder={offerTable.columnOrder.filter(
            (key) => offerTable.visibleColumns[key]
          )}
        >
          <TableHeader>
            <TableRow>
              {offerTable.columnOrder
          ...
          <TableBody>
            {isLoading ? (
              <TableRow>
                <TableCell
                  colSpan={
                    offerTable.columnOrder.filter((k) => offerTable.visibleColumns[k]).length || 1
                  }
                  className="p-0"
                >
                  <TableSkeleton
                    rows={5}
                    columns={
                      offerTable.columnOrder.filter((k) => offerTable.visibleColumns[k]).length ||
                      10
                    }
                  />
                </TableCell>
                      ...
                      switch (colKey) {
                        ...
                        case 'contactedAt':
                          return (
                            ...
                            >
                              {formatStatusTime(lead.contactedAt)}
                            </FrozenTableCell>
                          );
                        case 'confirmedAt':
                          return (
                            <FrozenTableCell
                              ...
                              className="text-xs text-muted-foreground"
                            >
                              {formatStatusTime(lead.confirmedAt)}
                            </FrozenTableCell>
                          );
                        case 'attendedAt':
                          return (
                            <FrozenTableCell
                              ...
                              className="text-xs text-muted-foreground"
                            >
                              {formatStatusTime(lead.attendedAt)}
                            </FrozenTableCell>
                          );
                        case 'completedAt':
                          return (
                            <FrozenTableCell
                              ...
                              className="text-xs text-muted-foreground"
                            >
                              {formatStatusTime(lead.completedAt)}
                            </FrozenTableCell>
                          );
                        case 'cancelledAt':
                          return (
                            <FrozenTableCell
                              ...
                              className="text-xs text-muted-foreground"
                            >
                              {formatStatusTime(lead.cancelledAt)}
                            </FrozenTableCell>
                          );
                        case 'date':
                          return (
                        ...
                        case 'comments':
                          return (
                            <FrozenTableCell key={colKey} columnKey={colKey}>
                              <CommentCount entityType="offerLead" entityId={lead.id} />
                            </FrozenTableCell>
                          );
                        case 'tasks':
                          return (
                            <FrozenTableCell key={colKey} columnKey={colKey}>
                              <TaskCount entityType="offerLead" entityId={lead.id} />
                            </FrozenTableCell>
                          );
                        case 'whatsapp':
                          return (
                            <FrozenTableCell key={colKey} columnKey={colKey}>
                              <WhatsAppStatusBadge entityType="offer_lead" entityId={lead.id} />
                            </FrozenTableCell>
                          );
                        case 'actions':
                          return (
                                ...
                                <AlertDialog>
                                  ...
                                  <AlertDialogContent>
                                    <AlertDialogHeader>
                                      ...
                                      <AlertDialogDescription>
                                        هل أنت متأكد من حذف هذا الحجز؟ لا يمكن التراجع عن هذا
                                        الإجراء.
                                      </AlertDialogDescription>
                                    </AlertDialogHeader>
                                    <AlertDialogFooter>
              ...
              <Tabs defaultValue="info" className="flex-1 overflow-hidden flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto mt-4">
                  ...
                  <TabsContent value="comments" className="mt-0">
                    <CommentsSection entityType="offerLead" entityId={selectedLead.id} />
                  </TabsContent>

                  <TabsContent value="tasks" className="mt-0">
                    <TasksSection entityType="offerLead" entityId={selectedLead.id} />
                  </TabsContent>

                  <TabsContent value="history" className="mt-0">
                    <AuditLogSection entityType="offerLead" entityId={selectedLead.id} />
                  </TabsContent>
                </div>
              </Tabs>
