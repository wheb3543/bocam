...
export function useTableFeatures({
  ...
  const saveColumnWidthsFn = useCallback(
  ...
  const saveFrozenColumnsFn = useCallback(
    (frozen: string[]) => {
  ...
  const defaultTemplates = useMemo(
    () => getDefaultTemplates(columns, tableKey),
    [columns, tableKey]
  );
  ...
  const createSharedTemplateMutation = trpc.sharedTemplates.create.useMutation({
    onSuccess: () => {
      utils.sharedTemplates.list.invalidate({ tableKey });
    },
  });
  ...
  const handleSaveSharedTemplate = useCallback(
    (
      name: string,
      cols: Record<string, boolean>,
      order: string[],
  ...
  const handleDeleteSharedTemplate = useCallback(
    (dbId: number) => {
