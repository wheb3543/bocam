          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="ghost"
                size="sm"
                className="h-6 text-[var(--text-sm)] text-white/80 hover:text-white hover:bg-white/10 w-full justify-start gap-2"
                aria-label="عرض البحثات المحفوظة"
              >
...
function WhatsAppContent() {
  ...
  const { data: autoReplyRules, refetch: refetchAutoReplyRules } =
    trpc.whatsapp.getAutoReplyRules.useQuery();
  ...
  const handleNewMessageTemplateIdChange = useCallback(
    (v: number | null) => setNewMessageTemplateId(v),
    []
  );
    ...
    >
      <div className="container mx-auto p-2 sm:p-4 md:p-6 max-w-7xl">
            ...
            <div className="flex gap-0.5 sm:gap-1 md:gap-2 items-center flex-shrink-0">
              ...
              <Link href="/admin/whatsapp/analytics">
                <Button
                  ...
                  size="sm"
                  className="gap-1 text-[10px] sm:text-[var(--text-xs)] md:text-[var(--text-sm)] h-6 sm:h-7 md:h-8 px-1 sm:px-1.5 md:px-2.5 hidden md:flex"
                  aria-label="التحليلات والإحصائيات"
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                ...
                {confirmDialogAction?.action === 'unarchive' &&
                  'هل أنت متأكد من إلغاء أرشفة هذه المحادثة؟'}
                {confirmDialogAction?.action === 'bulk-archive' &&
                  `هل أنت متأكد من أرشفة ${confirmDialogAction?.ids?.length || 0} محادثة؟`}
                {confirmDialogAction?.action === 'bulk-important' &&
                  `هل أنت متأكد من تعيين ${confirmDialogAction?.ids?.length || 0} محادثة كمهمة؟`}
                {confirmDialogAction?.action === 'delete' &&
                  'هل أنت متأكد من حذف هذه المحادثة وجميع رسائلها؟ لا يمكن التراجع عن هذا الإجراء.'}
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
              ...
              <Button
                ...
                disabled={
                  ...
                  bulkMarkImportantMutation.isPending ||
        ...
        >
          ...
          >
            <div
              className={`border-l dark:border-gray-800 h-full overflow-hidden flex flex-col transition-all duration-300 w-full opacity-100`}
            >
