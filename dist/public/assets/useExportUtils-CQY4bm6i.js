...
`});const d=new Blob(["\uFEFF"+a],{type:"text/csv;charset=utf-8;"}),o=document.createElement("a"),r=URL.createObjectURL(d);o.setAttribute("href",r),o.setAttribute("download",h||`${s.tableName}_${Date.now()}.csv`),o.style.visibility="hidden",document.body.appendChild(o),o.click(),document.body.removeChild(o),m.success("تم التصدير إلى CSV بنجاح")}async function W(l){const{metadata:s,columns:u,data:f,filename:h}=l,a=m.loading("جاري إنشاء ملف PDF...");try{const d=s.filters?Object.fromEntries(Object.entries(s.filters).map(([v,g])=>[v,String(g)])):void 0,r=await D.useUtils().client.export.generatePDF.mutate({metadata:{...s,filters:d},columns:u,data:f});if(!r.success||!r.pdf)throw new Error("فشل إنشاء ملف PDF");const n=atob(r.pdf),p=new Uint8Array(n.length);for(let v=0;v<n.length;v++)p[v]=n.charCodeAt(v);const i=new Blob([p],{type:"application/pdf"}),c=document.createElement("a"),x=URL.createObjectURL(i);c.setAttribute("href",x),c.setAttribute("download",h||`${s.tableName}_${Date.now()}.pdf`),c.style.visibility="hidden",document.body.appendChild(c),c.click(),document.body.removeChild(c),URL.revokeObjectURL(x),m.success("تم التصدير إلى PDF بنجاح",{id:a})}catch(d){throw console.error("PDF export error:",d),m.error("حدث خطأ أثناء التصدير إلى PDF",{id:a}),d}}function Y(l){const{metadata:s,columns:u,data:f}=l,h=window.open("","_blank");if(!h){m.error("فشل فتح نافذة الطباعة. يرجى السماح بالنوافذ المنبثقة.");return}let a="";if(s.dateRange&&(a+=`خلال الفترة من ${s.dateRange}`),s.filters&&Object.keys(s.filters).length>0){const i=Object.entries(s.filters).map(([c,x])=>`${c}: ${x}`).join(" - ");a?a+=" - "+i:a=i}const d=u.length,o=d<=5?"portrait":"landscape",r=d<=5?"11pt":d<=8?"10pt":"9pt",n=d<=5?"10pt":d<=8?"9pt":"8pt",p=`
    ...
    <body>
      ...
      <div class="page-footer">
        <div class="footer-left"><p>وقت الطباعة: ${X(new Date)}</p></div>
  ...
  `;h.document.write(p),h.document.close(),m.success("تم فتح نافذة الطباعة")}async function q(l){try{switch(l.format){case"excel":await H(l);break;case"csv":Q(l);break;case"pdf":await W(l);break;default:throw new Error("تنسيق غير مدعوم")}}catch(s){throw console.error("Export error:",s),m.error("حدث خطأ أثناء التصدير"),s}}function le(l){const{user:s}=O(),u=S.useCallback((o,r,n)=>({tableName:l.tableName,dateRange:n,filters:r&&Object.keys(r).length>0?r:void 0,totalRecords:o.length,exportedRecords:o.length,exportDate:new Date().toLocaleString("ar-SA"),exportedBy:s?.name||"مستخدم"}),[l.tableName,s?.name]);S.useCallback((o,r)=>r?Object.entries(r).filter(([n,p])=>p).map(([n])=>o.find(i=>i.key===n)||{key:n,label:n}).filter(Boolean):o,[]);const f=S.useCallback(async(o,r)=>{const{data:n,activeFilters:p,dateRangeStr:i,visibleColumns:c}=r;if(!n||n.length===0){m.error("لا توجد بيانات للتصدير");return}try{const x=u(n,p,i),v=n.map(e=>l.mapToExportRow(e));let g;c?g=Object.entries(c).filter(([e,b])=>b).map(([e])=>l.exportColumns.find(N=>N.key===e)||{key:e,label:e}):g=l.exportColumns;const F=`${l.filenamePrefix}_${Date.now()}.${o==="excel"?"xlsx":o}`;await q({format:o,metadata:x,columns:g,data:v,filename:F}),m.success(`تم تصدير البيانات بنجاح بتنسيق ${o.toUpperCase()}`)}catch(x){console.error("Export error:",x),m.error("حدث خطأ أثناء التصدير")}},[u,l]),h=S.useCallback(o=>{const{data:r,activeFilters:n,dateRangeStr:p,visibleColumns:i}=o;if(!r||r.length===0){m.error("لا توجد بيانات للطباعة");return}try{const c=u(r,n,p),x=l.mapToPrintRow||l.mapToExportRow,v=r.map(e=>x(e)),g=l.printColumns||l.exportColumns;let F;i?F=Object.entries(i).filter(([e,b])=>b).map(([e])=>g.find(N=>N.key===e)||{key:e,label:e}):F=g,Y({format:"pdf",metadata:c,columns:F,data:v})}catch(c){console.error("Print error:",c),m.error("حدث خطأ أثناء الطباعة")}},[u,l]),a=S.useCallback(o=>{const r={};for(const n of o){if(!n.value)continue;const p=Array.isArray(n.value)?n.value.join(", "):n.value;p&&(r[n.label]=p)}return r},[]),d=S.useCallback((o,r)=>`${o.toLocaleDateString("ar-SA")} - ${r.toLocaleDateString("ar-SA")}`,[]);return{handleExport:f,handlePrint:h,buildActiveFilters:a,formatDateRange:d}}export{re as S,ne as T,le as u};
