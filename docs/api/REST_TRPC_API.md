...
يوضح هذا الدليل تاريخيًا بعض مواصفات وهيكلة واجهات التطبيق البرمجية. المرجع التشغيلي الأحدث المطابق للكود هو [مرجع API وWebhooks الحالي](./API_RUNTIME_REFERENCE.md). يجب مراجعة أي endpoint أو payload هنا مقابل الراوتر والاختبار قبل استخدامه.
...
1. **`publicProcedure`:** واجهات عامة يمكن استدعاؤها بدون مصادقة.
...
4. بوابة المريض تستخدم middleware محليًا داخل `server/routers/patientPortal.ts`؛ لا يوجد `patientProcedure` عام صادر من `server/_core/trpc.ts`.
