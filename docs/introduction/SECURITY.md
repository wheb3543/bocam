...
> **حالة التحقق:** هذا المستند سياسة عامة، وليس إثباتًا آليًا لكل ادعاء أمني. المرجع التنفيذي الحالي للمصادقة والصلاحيات هو [مرجع الهوية والصلاحيات](../AUTHENTICATION_RBAC.md)، ويجب اعتبار الادعاءات غير المرتبطة بمصدر كود أو اختبار `needs-verification` حتى تراجع في مرحلة الأمان المختصة.

## التنفيذ الحالي الموثق

- توجد مصادقة محلية username/password ومصادقة OAuth/Manus بمسارين للجلسات.
- توجد حماية tRPC عبر `protectedProcedure` و`adminProcedure` و`permissionProcedure`.
- توجد صلاحيات تفصيلية وأدوار أساسية وتوافق legacy في `shared/rolePermissions.ts` و`rolePermissionService.ts`.
- توجد حواجز ميزات ترخيص في الواجهة، وهي منفصلة عن RBAC ولا تستبدل حماية الخادم.
- تفاصيل اختلاف أسرار الجلسات وخصائص Cookie والفجوات الحالية موثقة في المرجع التنفيذي.

الادعاءات المتعلقة بـCSRF و2FA وHIPAA وGDPR والتشفير الشامل تحتاج مرجع تنفيذ واختبار مستقل قبل وصفها كميزات مؤكدة.
