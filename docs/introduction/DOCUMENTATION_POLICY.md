...
## 9. حوكمة الوثائق ومصفوفة التغطية

يجب أن يكون لكل ملف توثيق سجل في `docs/DOCUMENTATION_REGISTRY.json` يحدد المسار والعنوان والمجال والنوع والحالة والجمهور والمالك ومصادر الكود والاختبارات والوثائق المرتبطة وتاريخ المراجعة. يُعد السجل خط أساس قابلًا للتوليد، ولا يُستخدم اسم الملف وحده لإثبات التغطية.

الحالات المعتمدة هي: `canonical` للمرجع الحالي، `working` للعمل الجاري، `deprecated` للوثيقة المستبدلة، `archived` للتاريخية، `generated` للمخرجات المولدة، و`unclassified` لما لم يُراجع بعد.

تُربط التغييرات المهمة في الكود بالوثائق المتأثرة عبر `docs/DOCUMENTATION_COVERAGE_MATRIX.md`. عند وجود تعارض بين وثيقتين أو بين الوثيقة والكود، يُسجل التعارض في `docs/DOCUMENTATION_CONFLICTS.md` ولا يتم التخمين أو الحذف.

لا يُطلب تعليق أو JSDoc لكل سطر أو دالة بسيطة بلا قيمة توضيحية. التوثيق إلزامي للأسطح العامة، العقود، منطق الأعمال المعقد، القيود، الأخطاء، وقرارات التصميم؛ أما الكود الواضح فيُترك دون تعليق زائد.

يجب أن تكون أمثلة الوثائق خالية من الأسرار والبيانات الشخصية، وأن تكون الروابط المحلية نسبية وقابلة للفحص. يُشغل `pnpm docs:check` عند تغيير سجل الوثائق أو فهرسها، ويُستخدم فحص الروابط الموجود في CI للتحقق من الروابط الخارجية والمحلية.
