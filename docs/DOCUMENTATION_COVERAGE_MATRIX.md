...
| **16** | **الواجهة وPWA وإمكانية الوصول** | `client/src/components/ui/`, `layout/`, `hooks/`, `contexts/`, `PWAManager` | `accessibility.test.tsx`, `darkMode.test.ts`, `adminLayoutVerification.test.ts` | [FRONTEND_PLATFORM_RUNTIME_REFERENCE.md](./domains/FRONTEND_PLATFORM_RUNTIME_REFERENCE.md) | `canonical` | Frontend Platform Team |
