...
- **Backend (`server/`)**: Express + tRPC 11, Drizzle ORM (`drizzle/schema/`), MySQL/TiDB, Redis caching & BullMQ queues in a symmetrical modular architecture:
...
5. **Database**: Always manage schema/migrations via Drizzle in `drizzle/` (modular tables in `drizzle/schema/`); never alter DB directly.
6. **Environment & Verification**: Ensure `.env` is valid (`scripts/qa/check-env.mjs`); run the narrowest relevant validation command (type-check, lint, or targeted test) before finishing.