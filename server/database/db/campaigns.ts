...
export async function linkOffersToCampaign(campaignId: number, offerIds: number[]) {
  ...
  if (offerIds.length > 0) {
    await db.insert(campaignOffers).values(offerIds.map((offerId) => ({ campaignId, offerId })));
  }
...
export async function linkCampsToCampaign(campaignId: number, campIds: number[]) {
  ...
  // Add new links
  if (campIds.length > 0) {
    await db.insert(campaignCamps).values(campIds.map((campId) => ({ campaignId, campId })));
  }
...
export async function linkDoctorsToCampaign(campaignId: number, doctorIds: number[]) {
  ...
  if (doctorIds.length > 0) {
    await db
      .insert(campaignDoctors)
      .values(doctorIds.map((doctorId) => ({ campaignId, doctorId })));
  }
...
export async function getCampaignsOverview() {
  ...
  const budgetResult = await db.select({ total: sql<number>`SUM(plannedBudget)` }).from(campaigns);
