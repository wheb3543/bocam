...
export const phoneQualityRouter = router({
...
export const conversationQualityRouter = router({
  getHistory: protectedProcedure
    .input(
      z
        .object({
    ...
    .query(async ({ input }) => {
      ...
      return query.orderBy(desc(whatsappConversationQuality.createdAt)).limit(input?.limit || 100);
    }),
});
