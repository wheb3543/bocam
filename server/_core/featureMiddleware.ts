 ...
 */
export const requireFeature = (features: string | string[], options: FeatureOptions = {}) => {
    ...
    if (requireAll) {
      ...
      if (!allEnabled) {
        ...
        throw new TRPCError({
          code: 'FORBIDDEN',
          message:
            errorMessage || `Feature(s) required but not enabled: ${missingFeatures.join(', ')}`,
        });
      }
    } else {
      ...
      if (!anyEnabled) {
        throw new TRPCError({
          code: 'FORBIDDEN',
          message:
            errorMessage || `None of the required features are enabled: ${featureArray.join(', ')}`,
        });
      }
    }
