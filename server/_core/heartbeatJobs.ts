...
export async function createHeartbeatJob(
  ...
  if (!ENV.forgeApiUrl || !ENV.forgeApiKey) {
    // Local fallback when forge is not configured
    const simulatedUid = `local_cron_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    return { taskUid: simulatedUid, nextExecutionAt: null };
  }
...
/**
 * Update an existing cron located by `taskUid`.
 */
export async function updateHeartbeatJob(
  ...
  if (patch.path !== undefined) {
    validateCallbackPath(patch.path);
  }
  if (!ENV.forgeApiUrl || !ENV.forgeApiKey) {
  ...
  if (patch.cron !== undefined) {
    body.cronExpression = patch.cron;
  }
  if (patch.path !== undefined) {
    body.callbackPath = patch.path;
  }
  if (patch.method !== undefined) {
    body.callbackMethod = patch.method;
  }
...
/**
 * List cron jobs owned by the resolved actor.
 */
