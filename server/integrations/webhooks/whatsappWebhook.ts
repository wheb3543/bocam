...
export function verifyWebhookSignature(req: Request): boolean {
  ...
  if (!appSecret) {
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.warn(
        '[WhatsApp Webhook] ⚠️  META_APP_SECRET not set — skipping signature verification (dev mode)'
      );
      return true;
    }
  ...
  if (!rawBody) {
    console.warn(
      '[WhatsApp Webhook] ⚠️  rawBody not available — ensure express.raw() middleware is applied'
    );
  ...
  if (type === 'APPOINTMENT') {
    ...
    if (appt?.phone) {
      ...
      dispatchWhatsAppMessage({
        ...
        console.error(
          `[WhatsApp Webhook] Failed to send ${triggerEvent} for appt ${bookingId}:`,
          err
        )
      );
    }
    ...
    if (lead?.phone) {
      ...
      dispatchWhatsAppMessage({
        ...
        console.error(
          `[WhatsApp Webhook] Failed to send ${triggerEvent} for offer ${bookingId}:`,
          err
        )
      );
    }
    ...
    if (reg?.phone) {
      ...
      dispatchWhatsAppMessage({
        ...
        console.error(
          `[WhatsApp Webhook] Failed to send ${triggerEvent} for camp reg ${bookingId}:`,
          err
        )
      );
    }
...
async function handleIncomingMessage(
  ...
  try {
    ...
    console.log(
      `[WhatsApp Webhook] 📩 Incoming ${type} message from ${from} (msgId: ${messageId})`
    );
    ...
    if (identity) {
      identityData = {
        ...
        createdTimestamp: identity.created_timestamp
          ? new Date(identity.created_timestamp).toISOString()
          : undefined,
        hash: identity.hash,
      };
    ...
    console.log(
      `[WhatsApp Webhook] Debug: conversation.id = ${conversation.id}, type = ${typeof conversation.id}`
    );
    console.log(
      `[WhatsApp Webhook] Debug: Saving message to DB - content: ${content}, type: ${messageType}`
    );
...
async function handleMessageStatus(
  ...
  try {
    ...
    console.log(
      `[WhatsApp Webhook] 📊 Message ${messageId} → ${messageStatus} (to: ${recipient_id})`
    );
    ...
    if (conversation) {
      conversationData = {
        ...
        expirationTimestamp: conversation.expiration_timestamp
          ? new Date(parseInt(conversation.expiration_timestamp) * 1000).toISOString()
          : undefined,
    ...
    if (messageStatus === 'failed' && hasMessageErrors) {
      ...
      if (errorCode === 131047) {
        console.warn(
          `[WhatsApp Webhook] ⚠️  24-hour window expired for ${recipient_id} — template required`
        );
      }
    }
    ...
    console.log(
      `[WhatsApp Webhook] ✅ Updated notification status: ${messageId} → ${messageStatus}`
    );
      ...
      try {
        await db
          ...
          .set({
            ...
            expirationTimestamp: conversationData.expirationTimestamp
              ? new Date(conversationData.expirationTimestamp)
              : undefined,
      ...
      publish('global:whatsapp', 'message_status_update', {
        ...
        deliveredAt:
          messageStatus === 'delivered'
            ...
            : undefined,
        readAt:
          messageStatus === 'read' ? new Date(parseInt(timestamp) * 1000).toISOString() : undefined,
        errorCode,
        errorTitle,
...
async function handleTemplateStatusUpdate(
  ...
  try {
    ...
    console.log(
      `[WhatsApp Webhook] 📋 Template "${message_template_name}" → ${event}${reason ? ` (${reason})` : ''}`
    );
    ...
    if (event === 'REJECTED' || event === 'DISABLED') {
      console.error(
        `[WhatsApp Webhook] ⛔ Template "${message_template_name}" ${event}: ${reason || 'No reason provided'}`
      );
    }
...
async function handleTemplateEvent(templateEvent: { id?: string; event?: string }) {
  try {
    ...
    if (Object.keys(updateData).length > 0) {
      ...
      console.log(
        `[WhatsApp Webhook] ✅ Updated template status for ${metaTemplateId} (${eventType})`
      );
    }
...
async function handleMessagingProductUpdate(productEvent: { phone_number?: string; event?: string }) {
  try {
    ...
    console.log(
      `[WhatsApp Webhook] 📨 Messaging product update: ${eventType} for ${phoneNumber || 'unknown'}`
    );
            ...
            if (value.messages && Array.isArray(value.messages)) {
              for (const message of value.messages) {
                ...
                if (message.errors && Array.isArray(message.errors)) {
                  console.error(
                    `[WhatsApp Webhook] ❌ Incoming message errors for ${message.from}:`
                  );
                  ...
                  if (message.type === 'unsupported') {
                    console.warn(
                      `[WhatsApp Webhook] ⚠️  Unsupported message type from ${message.from}`
                    );
                    continue;
                  }
                }
            ...
            if (value.statuses && Array.isArray(value.statuses)) {
              for (const status of value.statuses) {
                ...
                if (status.errors && Array.isArray(status.errors)) {
                  console.error(
                    `[WhatsApp Webhook] ❌ Outbound message status errors for ${status.id}:`
                  );
  ...
  return async (req: Request, res: Response) => {
    ...
    if (req.method === 'POST') {
      ...
      try {
        const body = req.body;
        console.log(
          `[WhatsApp Webhook] ✅ Received verified webhook event (object: ${body.object})`
        );
