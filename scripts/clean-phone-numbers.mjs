...
async function analyzeData(connection) {
  ...
  for (const [tableName, config] of Object.entries(TABLE_CONFIG)) {
    ...
    console.log(
      `   Needs sanitization: ${needsSanitization} (${analysisResults.sanitization[tableName].percentage}%)`
    );
    console.log(
      `   Needs normalization: ${needsNormalization} (${analysisResults.normalization[tableName].percentage}%)`
    );
  ...
  console.log(
    `   Total conversations affected: ${conversations.length - phoneMap.size + duplicateCount}`
  );
  console.log('');
}
...
async function executeSanitization(connection) {
  ...
  for (const [tableName, config] of Object.entries(TABLE_CONFIG)) {
    ...
    for (const row of rows) {
      ...
      if (originalPhone !== sanitizedPhone) {
        await connection.query(`UPDATE ${tableName} SET ${phoneColumn} = ? WHERE ${idColumn} = ?`, [
          sanitizedPhone,
          row[idColumn],
        ]);
        updated++;
...
async function executeDeduplication(connection) {
  ...
  for (const group of dedupStats.duplicateGroups) {
    ...
    for (const convToDelete of conversationsToDelete) {
      ...
      await connection.query(`DELETE FROM whatsapp_conversations WHERE id = ?`, [convToDelete.id]);
    }
...
function askConfirmation() {
  return new Promise((resolve) => {
    ...
    rl.question(
      '\n⚠️  Do you want to proceed with the sanitization and normalization? (yes/no): ',
      (answer) => {
        rl.close();
